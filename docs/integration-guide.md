# Octopus SDK for Unity — integration guide

The [README](../README.md) covers installation and a minimal quickstart. This guide is the
reference for everything else: connection modes, push notifications, groups, badges, theming,
locale, analytics, and the configuration of the `UnityExample` sample. Cross-platform concepts
(SSO, token signing, content options) are documented on
[doc.octopuscommunity.com](https://doc.octopuscommunity.com).

## Known issue: Xcode 27 and iOS 27
...
not change the package's minimum Unity version (2019.4).

## Initialization and user connection
...
## Sample configuration (`UnityExample`)
...
- **Profiles.** Default, Forced Login and Managed Fields. Forced Login and Managed Fields reuse
  the Default API key unless you set their own. Leave `userId` empty to use the built-in test
  identities, or fill the identity fields to use your own test users.
- **Connection presets.** With `ssoTokenSecret` set, the sample signs a fresh HS256 token per
  preset: no entitlements, `customer:premium`, `customer:moderator`, or both.
- **Without a secret.** Each profile's static `authToken` is used as is, so presets cannot change
  its claims. With neither a secret nor a token, Connect explains what is missing instead of
  calling `ConnectUser`.
- **Force login** switches to the Forced Login profile. **Lifecycle > Switch community** needs a
  second community key in Forced Login > Api Key; with only the Default key it refuses to run.
